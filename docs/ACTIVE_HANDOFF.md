...
## Current Status
  ...
  - dev deployment now runs `collection-worker` as an image Lambda with Crawl4AI fallback enabled; the deployed image uses Playwright/Chromium and keeps the fallback strategy `baseline HTTP first -> Crawl4AI cleaned_html fallback -> rendered raw_html fallback`
  - Lambda rejected the first Docker Desktop push because the image manifest format was unsupported; `scripts/build_collection_worker_image.ps1` now builds with `docker buildx`, `--platform linux/amd64`, `--provenance=false`, and `--sbom=false` so ECR images stay Lambda-compatible
