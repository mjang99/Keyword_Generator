...
| OCR policy | Run screenshot OCR when decoded visible text is thin or charset confidence is low. Run asset OCR on ranked images only. Default cap is 8 page images per URL, min size `300x300`, reject sprites, logos, icons, and thumbnails, accept OCR blocks only when they contain `>=30` chars or at least two product/spec tokens. |
...
- `OCRQueue -> OCR Worker Lambda (container)`: PaddleOCR over ranked images and screenshots
...
- `OCR Worker Lambda` is a supplemental media-text stage only. It reads collection artifacts, runs PaddleOCR on screenshots and ranked images, filters OCR blocks, and writes an OCR supplement artifact. It never refetches the page, changes the page class, or calls Bedrock.
